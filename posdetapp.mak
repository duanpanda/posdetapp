...
# indicate a module must be built and named posdetapp.dll
BUILD_BREWMODS = posdetapp

# specifies all the C source files that need to be compiled
posdetapp_C_SRCS = AEEAppGen \
	AEEModGen \
	PosDetApp \
	RyanUtils

# specifies the cif files to be compiled
posdetapp_CIFS = posdetapp

# specifies the resource files to be compiled
posdetapp_CARS = posdetapp
...
BREWMP_INPUT = Debug/PosDetApp.dll \
	Debug/posdetapp.bar \
	Debug/PosDetApp.mif
...
BREWMP_INPUT = Release/PosDetApp.dll \
	Release/posdetapp.bar \
	Release/PosDetApp.mif

endif

ifneq ($(V_TARGET),Win32)
#Arm package settings
BREWMP_INPUT = $(OBJ_DIR)/PosDetApp.mod \
	$(OBJ_DIR)/posdetapp.bar \
	$(OBJ_DIR)/PosDetApp.mif
